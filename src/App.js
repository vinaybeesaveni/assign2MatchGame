...
class App extends Component {
  ...
  changeActiveTab = id => {
    const activeTabObj = tabsList.find(eachObj => eachObj.tabId === id)
    this.setState({activeTab: activeTabObj.tabId})
  }

  generateRandomImg = () => {
    const randomIndex = Math.ceil(Math.random() * 30)
    this.setState({activeImg: imagesList[randomIndex].imageUrl})
  }

  onThumbnailPress = pressedImg => {
    const {activeImg} = this.state
    if (activeImg === pressedImg) {
      this.setState(prevState => ({
  ...
  renderHomePage = () => {
    const {activeTab, activeImg} = this.state
    const finalThumbnails = imagesList.filter(
      eachImg => eachImg.category === activeTab,
    )
    return (
      <div className="game-home-container">
        <img src={activeImg} alt="match" className="match-image" />
        ...
        <ul className="thumbnails-container">
          {finalThumbnails.map(eachImage => (
            <TabItem
              key={eachImage.id}
              eachImg={eachImage}
              onThumbnailPress={this.onThumbnailPress}
            />
          ))}
        </ul>
      </div>
